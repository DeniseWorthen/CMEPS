...
!================================================================================
  ...
  subroutine esmFldsExchange_nems(gcomp, phase, rc)
    ...
    use esmflds               , only : mapconsf_aofrc
    ...
    ! Set normtype according to norm_mode for fields from ATM->OCN
    if (trim(norm_mode) == 'aofrac') then
      normtype = 'aofrac'
    else
      normtype = 'one'
    end if
    ...
    write(msgString,'(A)') trim(subname)//': normtype is '//trim(normtype)
    call ESMF_LogWrite(trim(msgString), ESMF_LOGMSG_INFO)
    ...
    if (trim(coupling_mode) == 'nems_orig' .or. trim(coupling_mode) == 'nems_frac') then
       ...
       do n = 1,size(flds)
          ...
          call addmap(fldListFr(compatm)%flds, 'Faxa_'//trim(flds(n)), compocn, mapconsf_aofrc, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_lwnet', compocn, mapconsf_aofrc, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_sen', compocn, mapconsf_aofrc, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_lat', compocn, mapconsf_aofrc, normtype, 'unset')
    else
       ! nems_orig_data
