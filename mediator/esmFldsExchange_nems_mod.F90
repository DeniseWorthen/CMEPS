  ...
  subroutine esmFldsExchange_nems(gcomp, phase, rc)
    ...
    if (trim(coupling_mode) == 'nems_orig' .or. trim(coupling_mode) == 'nems_frac') then
       ...
       do n = 1,size(flds)
          ...
          call addmap(fldListFr(compatm)%flds, 'Faxa_'//trim(flds(n)), compocn, mapconsf_aofrac, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_lwnet', compocn, mapconsf_aofrac, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_sen', compocn, mapconsf_aofrac, normtype, 'unset')
       ...
       call addmap(fldListFr(compatm)%flds, 'Faxa_lat', compocn, mapconsf_aofrac, normtype, 'unset')
    else
       ! nems_orig_data
