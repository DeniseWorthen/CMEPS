...
!-----------------------------------------------------------------------------
  ...
  subroutine SetServices(gcomp, rc)
    ...
    call ufs_trace_init()
    call ufs_trace("cmeps", "SetServices", "B")
    ...
    call ufs_trace("cmeps", "SetServices", "E")
  ...
  subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
    ...
    if (profile_memory) call ESMF_VMLogMemInfo("Entering "//trim(subname))
    call ufs_trace("cmeps", "InitializeP0", "B")
    call ESMF_GridCompGet(gcomp, vm=vm, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_VMGet(vm, localPet=localPet, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    call ufs_trace("cmeps", "InitializeP0", "E")
  ...
  subroutine AdvertiseFields(gcomp, importState, exportState, clock, rc)
    ...
    call ufs_trace("cmeps", "AdvertiseFields", "B")
    call ESMF_LogWrite(trim(subname)//": called", ESMF_LOGMSG_INFO)
    rc = ESMF_SUCCESS
    if (profile_memory) call ESMF_VMLogMemInfo("Entering "//trim(subname))
    ...
    call ufs_trace("cmeps", "AdvertiseFields", "E")
  ...
  subroutine RealizeFieldsWithTransferProvided(gcomp, importState, exportState, clock, rc)
    ...
    !-----------------------------------------------------------
    call ufs_trace("cmeps", "RealizeFieldsWithTransferProvided", "B")
    ...
    if (profile_memory) call ESMF_VMLogMemInfo("Leaving "//trim(subname))
    call ESMF_LogWrite(trim(subname)//": done", ESMF_LOGMSG_INFO)
    call ufs_trace("cmeps", "RealizeFieldsWithTransferProvided", "E")
  ...
  subroutine ModifyDecompofMesh(gcomp, importState, exportState, clock, rc)
    ...
    !-----------------------------------------------------------
    call ufs_trace("cmeps", "ModifyDecompofMesh", "B")
    ...
    if (profile_memory) call ESMF_VMLogMemInfo("Leaving "//trim(subname))
    call ESMF_LogWrite(trim(subname)//": done", ESMF_LOGMSG_INFO)
    call ufs_trace("cmeps", "ModifyDecompofMesh", "E")
  ...
  subroutine RealizeFieldsWithTransferAccept(gcomp, importState, exportState, clock, rc)
    ...
    !-----------------------------------------------------------
    call ufs_trace("cmeps", "RealizeFieldsWithTransferAccept", "B")
    ...
    if (profile_memory) call ESMF_VMLogMemInfo("Leaving "//trim(subname))
    call ESMF_LogWrite(trim(subname)//": done", ESMF_LOGMSG_INFO)
    call ufs_trace("cmeps", "RealizeFieldsWithTransferAccept", "E")
  ...
  subroutine DataInitialize(gcomp, rc)
    ...
    !debug
    str1='FirstCallF'
    if(first_call)str1='FirstCallT'
    call ufs_trace("cmeps", "DataInitialize", "B")
    ...
    if (first_call) then
       !call ufs_trace("cmeps", "DataInitialize"//trim(str1), "B")
      ...
      !---------------------------------------
      !call ufs_trace("cmeps", "RH_init"//trim(str1), "B")
      ...
      !call ufs_trace("cmeps", "RH_init"//trim(str1), "E")
      !---------------------------------------
      ! Initialized packed field data structures
      !---------------------------------------

      !call ufs_trace("cmeps", "Packing_init"//trim(str1), "B")
      do ndst = 1,ncomps
         do nsrc = 1,ncomps
      ...
      if ( ESMF_FieldBundleIsCreated(is_local%wrap%FBMed_ocnalb_o) .and. &
       ...
       !call ufs_trace("cmeps", "Packing_init"//trim(str1), "E")
      ...
      !---------------------------------------
      !call ufs_trace("cmeps", "DataInitialize"//trim(str1), "E")
      first_call = .false.
      ...
      call ufs_trace("cmeps", "DataInitialize", "E")
      RETURN
    ...
    !call ufs_trace("cmeps", "fraction_init", "B")
    ...
    !call ufs_trace("cmeps", "fraction_init", "E")
    !----------------------------------------------------------
    ...
    !call ufs_trace("cmeps", "ATMinitDataInitialize", "B")
    ...
    !call ufs_trace("cmeps", "ATMinitDataInitialize", "E")
    ...
    allDone = .true.
    str2 = 'allDoneF'
    if(allDone) str2='allDoneT'
    !call ufs_trace("cmeps", "DataInitialize"//trim(str2), "B")
    ...
    if (allDone) then
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       !call ufs_trace("cmeps", "DataInitialize"//trim(str2), "E")
       !call ufs_trace("cmeps", "DataInitialize", "E")
    else ! Not all done
       call NUOPC_CompAttributeSet(gcomp, name="InitializeDataComplete", value="false", rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
